...
public class CallLogAdapter extends BaseAdapter {
    ...
    private List<CuocGoi> listtimkiem;
    private int layout;
    ...
    public void filter(String charText) {
        ...
        if (charText.length() == 0) {
            listtimkiem.addAll(listdata);
        }
        else {
            for (int i= 0; i<listdata.size();i++) {
                if (listdata.get(i).getTen().toLowerCase(Locale.getDefault()).contains(charText.toLowerCase())) {
                    listtimkiem.add(listdata.get(i));
                }
            }
        }
        notifyDataSetChanged();
    }
    public List<CuocGoi> getHistories(){
        return listtimkiem;
    }
